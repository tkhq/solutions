...
type RecommendedStep = StepConfig & { reason: string }
...
function getRecommendation(answers: Answers): Recommendation {
  ...
  if (appType === 'institutional' || appType === 'treasury') {
    parentSteps.push({
  ...
  if (restrictions !== 'later') {
    if (restrictions === 'none') {
      parentSteps.push({
        kind: 'CREATE_POLICY',
        title: 'Create Org Policy',
        description: 'Create a top-level policy governing signing permissions across your organization.',
        params: { type: 'permissive' },
        reason: 'Create a permissive policy to allow all signing operations in your org.',
      })
    } else if (restrictions === 'allowlist') {
      parentSteps.push({
        kind: 'CREATE_POLICY',
        title: 'Create Org Policy',
        description: 'Create a top-level policy governing signing permissions across your organization.',
        params: { type: 'allow-all' },
        reason: "Create a base policy \u2014 you'll add address restrictions per sub-org.",
      })
    } else if (restrictions === 'sign-only') {
      parentSteps.push({
        kind: 'CREATE_POLICY',
        title: 'Create Org Policy',
        description: 'Create a top-level policy governing signing permissions across your organization.',
        params: { type: 'sign-only' },
        reason: 'Restrict to signing operations only \u2014 prevents any admin actions from your API user.',
      })
    }
  }
  ...
  if (restrictions !== 'later') {
    if (restrictions === 'none') {
      subOrgSteps.push({
        ...
        params: { type: 'permissive' },
        reason: 'Allow all signing in this sub-org \u2014 suitable for a managed custodial setup.',
      })
    } else if (restrictions === 'allowlist') {
      subOrgSteps.push({
        kind: 'CREATE_POLICY',
        ...
        description: 'Create a signing policy scoped to this sub-org.',
        params: { type: 'permissive' },
        reason: "Create a policy you'll customize with the end user's allowed addresses.",
      })
    } else if (restrictions === 'sign-only') {
      subOrgSteps.push({
        kind: 'CREATE_POLICY',
        ...
        description: 'Create a signing policy scoped to this sub-org.',
        params: { type: 'sign-only' },
        reason: 'Restrict this sub-org to signing only \u2014 prevents wallet creation or user changes.',
      })
    }
  }
  ...
  let summary: string
  if (appType === 'treasury') {
    summary = 'Internal treasury setup with strict key tagging and signing restrictions for maximum control.'
  } else if (appType === 'gaming') {
    summary = 'Gaming or NFT platform setup. Wallets are created per user with policies to control what assets can be moved.'
...
export default function RecommendPage() {
  ...
  function launchParent(steps: RecommendedStep[]) {
    const stepConfigs: StepConfig[] = steps.map(({ reason: _reason, ...s }) => s)
