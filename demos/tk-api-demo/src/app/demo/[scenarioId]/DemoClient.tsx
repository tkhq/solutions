...
function EditableRequestPanel({
  ...
  return (
    <div className={`rounded-xl overflow-hidden border transition-colors ${parseError ? 'border-red-400 dark:border-red-600' : 'border-violet-300 dark:border-violet-700'}`}>
      <div className="flex items-center justify-between px-4 py-2 bg-gray-100 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700/60">
        <div className="flex items-center gap-2">
      ...
      <div className="relative">
        <textarea
          ...
          spellCheck={false}
          className="w-full p-4 text-xs font-mono leading-relaxed bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 resize-none focus:outline-none min-h-48"
...
export default function DemoClient({ scenario, customSteps }: { scenario: Scenario; customSteps?: import('@/types/scenario').StepConfig[] }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/preview', {
      ...
      body: JSON.stringify({ scenarioId: scenario.id, stepIndex: currentStep, sessionState, customSteps }),
    })
      .then((r) => r.json())
      .then((d) => {
  ...
  const runStep = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scenarioId: scenario.id, stepIndex: currentStep, sessionState, customSteps, overrideRequest }),
      })
  ...
  }, [currentStep, editedRequest, scenario.id, sessionState])
          ...
          <div className="mb-6 max-w-3xl">
            <div className="flex items-center gap-2 mb-1">
              ...
              {currentStepState.status === 'success' && (
                scenario.steps[currentStep].kind === 'SIGN_TRANSACTION' ||
                scenario.steps[currentStep].kind === 'SIGN_RAW_PAYLOAD'
              ) && (
